...
void SetLogLevel(const std::string& log_level) {
  if (log_level == "trace") {
    ...
    quiver::util::config::SetLogLevel(quiver::util::config::LogLevel::kInfo);
  } else if (log_level == "warn") {
    quiver::util::config::SetLogLevel(quiver::util::config::LogLevel::kWarn);
  } else {
    ThrowNotOk(quiver::Status::Invalid("Invalid log level: " + log_level));
  }
}
...
PYBIND11_MODULE(_quiver, mod) {
  ...
  mod.def("_set_log_level", SetLogLevel, "Sets the log level", pybind11::arg("log_level"));
