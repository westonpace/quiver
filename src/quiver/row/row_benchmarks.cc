...
void BenchEncode(benchmark::State& state, Storage* storage) {
  const TestData& test_data = GetTestData();

  int64_t num_threads = state.range(0);

  std::unique_ptr<row::RowEncoder> encoder;
  row::RowEncoder::Create(&test_data.schema, storage, &encoder).AbortNotOk();

  int64_t num_rows = test_data.batch->length();
  auto batch_size =
      static_cast<int32_t>(bit_util::RoundUpToPowerOf2(num_rows / num_threads, 8));
...
#pragma omp parallel
    {
      ...
      if (!util::Tracer::GetCurrent()->InOperation()) {
        task_scope =
            util::Tracer::GetCurrent()->StartOperation(util::tracecat::kUnitTest);
      }
      int32_t start = batch_size * omp_get_thread_num();
      auto remaining = static_cast<int32_t>(num_rows - static_cast<int64_t>(start));
      int32_t length = std::min(batch_size, remaining);
      std::unique_ptr<ReadOnlyBatch> batch;
      AssertOk(test_data.batch->SliceView(start, length, &batch));

      int64_t row_id = -1;
      encoder->Append(*batch, &row_id).AbortNotOk();
    }
...
void BM_EncodeMemory(benchmark::State& state) {
  ...
  BenchEncode(state, storage.get());
}

void BM_EncodeFile(benchmark::State& state) {
  std::unique_ptr<Storage> storage = TmpFileStorage(/*direct_io=*/false);
  BenchEncode(state, storage.get());
}
...
void BenchDecodeMemory(const datagen::GeneratedData& data, benchmark::State& state,
  ...
  std::unique_ptr<row::RowEncoder> encoder;
  row::RowEncoder::Create(data.schema.get(), storage.get(), &encoder).AbortNotOk();
...
BENCHMARK(quiver::BM_EncodeMemory)->ArgName("NumThreads")->Arg(1)->Arg(4)->Arg(8);
BENCHMARK(quiver::BM_EncodeFile)->ArgName("NumThreads")->Arg(1)->Arg(4)->Arg(8);
