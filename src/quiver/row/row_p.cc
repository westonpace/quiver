...
class RowEncoderImpl : public RowEncoder {
 public:
  ...
  Status Append(const ReadOnlyBatch& batch, int64_t* out_row_id) override {
    ...
    int64_t offset_bytes;
    {
      ...
      *out_row_id = row_id_counter_;
      ...
      offset_bytes = offset_bytes_;
      offset_bytes_ += chunk_bytes;
    }

    // Prepare
    std::vector<FlatEncoder> flat_encoders;
    flat_encoders.reserve(schema_.schema->num_fields());
    for (int32_t field_idx = 0; field_idx < schema_.schema->num_fields(); field_idx++) {
      flat_encoders.emplace_back(schema_.schema->field(field_idx));
      flat_encoders.back().Prepare(batch.array(field_idx));
    }

    // Encode
    SinkBuffer sink_buffer;
    QUIVER_RETURN_NOT_OK(sink_->ReserveChunkAt(offset_bytes, chunk_bytes, &sink_buffer));
    SPDLOG_TRACE("Appending batch of {} bytes to offset {}", chunk_bytes, offset_bytes);
  ...
  [[nodiscard]] uint32_t row_width() const override { return schema_.fixed_length; }
...
Status RowEncoder::Create(const SimpleSchema* schema, Storage* storage,
  ...
  if (storage->requires_alignment()) {
    return Status::NotImplemented("Alignment-respecting encoder");
  }
