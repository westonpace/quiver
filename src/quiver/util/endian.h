...
#define QUIVER_LITTLE_ENDIAN 1
#else
#define QUIVER_LITTLE_ENDIAN 0
#endif
#endif

#if defined(_MSC_VER)
#include <intrin.h>  // IWYU pragma: keep
#define QUIVER_BYTE_SWAP64 _byteswap_uint64
#define QUIVER_BYTE_SWAP32 _byteswap_ulong
#else
#define QUIVER_BYTE_SWAP64 __builtin_bswap64
#define QUIVER_BYTE_SWAP32 __builtin_bswap32
#endif
...
static inline int64_t ByteSwap(int64_t value) { return QUIVER_BYTE_SWAP64(value); }
static inline uint64_t ByteSwap(uint64_t value) {
  return static_cast<uint64_t>(QUIVER_BYTE_SWAP64(value));
}
static inline int32_t ByteSwap(int32_t value) { return QUIVER_BYTE_SWAP32(value); }
static inline uint32_t ByteSwap(uint32_t value) {
  return static_cast<uint32_t>(QUIVER_BYTE_SWAP32(value));
}
static inline int16_t ByteSwap(int16_t value) {
  constexpr auto mask = static_cast<int16_t>(0xff);
  return static_cast<int16_t>(((value >> 8) & mask) | ((value & mask) << 8));
}
...
static inline double ByteSwap(double value) {
  const uint64_t swapped = QUIVER_BYTE_SWAP64(util::SafeCopy<uint64_t>(value));
  return util::SafeCopy<double>(swapped);
}
static inline float ByteSwap(float value) {
  const uint32_t swapped = QUIVER_BYTE_SWAP32(util::SafeCopy<uint32_t>(value));
...
static inline void ByteSwap(void* dst, const void* src, int len) {
  ...
  auto* dest = reinterpret_cast<uint8_t*>(dst);
  const auto* source = reinterpret_cast<const uint8_t*>(src);
  for (int i = 0; i < len; ++i) {
...
struct Reader {
  ...
  const T& operator[](size_t index) const {
    return native_array[QUIVER_LITTLE_ENDIAN ? index : N - 1 - index];
  }
};
