...
template <typename T>
constexpr T BitNot(T value) {
  return ~value;
}

template <>
constexpr bool BitNot(bool value) {
  return !value;
}
...
struct BitBlockCount {
  ...
  [[nodiscard]] bool NoneSet() const { return this->popcount == 0; }
  [[nodiscard]] bool AllSet() const { return this->length == this->popcount; }
};
 ...
 private:
  ...
  BitBlockCount NextWord() {
    ...
    if (bits_remaining_ < bits_required_to_use_words) {
      const auto run_length = static_cast<int16_t>(std::min(bits_remaining_, kWordBits));
      int16_t popcount = 0;
...
class OptionalBinaryBitBlockCounter {
 public:
...
static Status VisitBitBlocks(const uint8_t* bitmap, int64_t offset, int64_t length,
  ...
  OptionalBitBlockCounter bit_counter(bitmap, offset, length);
  int64_t position = 0;
