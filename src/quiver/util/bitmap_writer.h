 ...
 public:
  ...
  void Next() {
    ...
    if (bit_mask_ == 0) {
      ...
      if (QUIVER_LIKELY(position_ < length_)) {
...
class FirstTimeBitmapWriter {
 ...
 public:
  ...
  void AppendWord(uint64_t word, int64_t number_of_bits) {
    if (QUIVER_UNLIKELY(number_of_bits == 0)) {
      return;
    }
    ...
    if (bit_offset != 0) {
      ...
      if (QUIVER_UNLIKELY(number_of_bits < bits_to_carry)) {
        return;
      }
    ...
    int64_t bytes_for_word = ::quiver::bit_util::BytesForBits(number_of_bits);
 ...
 public:
  ...
  void PutNextTrailingByte(uint8_t byte, int valid_bits) {
    if (valid_bits == 8) {
      ...
      util::BitmapWriter writer(bitmap_, offset_, valid_bits);
      for (int i = 0; i < valid_bits; ++i) {
        (byte & 0x01) != 0 ? writer.Set() : writer.Clear();
        writer.Next();
        byte >>= 1;
      }
      writer.Finish();
 ...
 private:
  ...
  union {
    ...
    struct {
      ...
      std::array<uint8_t, sizeof(Word) - 1> padding_bytes_;
#endif
      uint8_t byte_;
    } epi;
  } current_data;
