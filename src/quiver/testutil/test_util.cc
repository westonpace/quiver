...
SchemaAndBatch TestBatch(std::vector<std::shared_ptr<arrow::Array>> arrays) {
  ...
  schema_and_batch.schema = std::make_unique<SimpleSchema>();
  ThrowNotOk(
      SimpleSchema::ImportFromArrow(c_data_schema.get(), schema_and_batch.schema.get()));
  ThrowNotOk(ImportBatch(c_data_arr.get(), schema_and_batch.schema.get(),
                         &schema_and_batch.batch));
