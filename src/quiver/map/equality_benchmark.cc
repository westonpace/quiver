...
void BM_EqualityEquals(benchmark::State& state) {
  ...
  datagen::GeneratedData data = datagen::Gen()->Field(datagen::Flat(field_width_bytes))
                                    ->NRows(kTotalSize);

  util::LocalAllocator alloc;
  util::local_ptr<std::span<uint8_t>> scratch = alloc.AllocateSpan<uint8_t>(bit_util::CeilDiv(kTotalSize, 8LL));

  std::unique_ptr<EqualityComparer> comparer = EqualityComparer::MakeSimpleBinaryEqualityComparer();

  for (auto _iter : state) {
    comparer->CompareEquality(data.batch->array(0), data.batch->array(0), *scratch).AbortNotOk();
    benchmark::DoNotOptimize(scratch->data());
    benchmark::ClobberMemory();
  }
  state.SetBytesProcessed(int64_t(state.iterations()) * data.batch->num_bytes() * 2);
}
...
void BM_EqualityUnequals(benchmark::State& state) {
  ...
  state.SetBytesProcessed(int64_t(state.iterations()) * lhs.batch->num_bytes() + rhs.batch->num_bytes());
}
...
BENCHMARK(quiver::map::BM_EqualityEquals)->ArgName("FieldWidthBytes")->Arg(1)->Arg(4)->Arg(8);
