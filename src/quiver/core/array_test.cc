...
TEST(ReadOnlyBatch, SelectView) {
  ...
  ASSERT_EQ(view->length(), data.batch->length());
  ASSERT_TRUE(array::ShallowEquals(view->array(0), data.batch->array(0)));
  ASSERT_TRUE(array::ShallowEquals(view->array(1), data.batch->array(2)));
}

TEST(ReadOnlyBatch, SliceView) {
  datagen::GeneratedData data = datagen::Gen()
                                    ->Field(datagen::Flat(3, 3))
                                    ->Field(datagen::Flat(2, 2))
                                    ->Field(datagen::Flat(8, 8))
  ...
  std::unique_ptr<ReadOnlyBatch> view;
  ASSERT_THAT(data.batch->SliceView(1, 10, &view).ToString(),
              ::testing::HasSubstr("must be divisible by 8"));

  AssertOk(data.batch->SliceView(8, 10, &view));
  ASSERT_EQ(view->schema(), data.schema.get());
  ASSERT_EQ(view->length(), 10);

  ReadOnlyFlatArray arr0 = std::get<ReadOnlyFlatArray>(view->array(0));
  ASSERT_EQ(arr0.length, 10);
}
