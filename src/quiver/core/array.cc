...
bool ShallowEquals(ReadOnlyArray lhs, ReadOnlyArray rhs) {
  if (ArrayLayout(lhs) != ArrayLayout(rhs)) {
    return false;
  }
  switch (ArrayLayout(lhs)) {
    case LayoutKind::kFlat: {
      ...
      if (lhs_flat.length != rhs_flat.length) {
        return false;
      }
      if (lhs_flat.validity.data() != rhs_flat.validity.data()) {
        return false;
      }
      if (lhs_flat.validity.size() != rhs_flat.validity.size()) {
        return false;
      }
      if (lhs_flat.values.data() != rhs_flat.values.data()) {
        return false;
      }
      return lhs_flat.values.size() == rhs_flat.values.size();
    }
    default:
      DCHECK(false) << "Not yet impelemented";
      return false;
  ...
  switch (ArrayLayout(array)) {
    case LayoutKind::kFlat: {
      ...
      if (!flat_array.validity.empty()) {
        ...
        validity = flat_array.validity.subspan(validity_byte_offset, validity_num_bytes);
      }
...
class ViewBatch : public ReadOnlyBatch {
 public:
  ViewBatch(const ReadOnlyBatch* target, std::vector<int32_t> column_indices,
            const SimpleSchema* view_schema, int32_t start, int32_t length)
      : target_(target),
        column_indices_(std::move(column_indices)),
        offset_(start),
        length_(length),
        view_schema_(view_schema) {}

  [[nodiscard]] ReadOnlyArray array(int32_t index) const override {
    // Unlikely case where we are viewing the entire batch
    if (column_indices_.empty() && offset_ == 0 && length_ < 0) {
      return target_->array(index);
    }
    ...
    if (column_indices_.empty()) {
      // Only slicing
      return array::Slice(target_->array(index), offset_, length_);
    }
    // Both column selection and slicing
    return array::Slice(target_->array(column_indices_[index]), offset_, length_);
  }

  [[nodiscard]] const SimpleSchema* schema() const override { return view_schema_; }
  ...
  [[nodiscard]] std::unique_ptr<ReadOnlyBatch> SelectView(
    ...
    return std::make_unique<ViewBatch>(target_, std::move(indices), new_schema, offset_,
                                       length_);
  }

  [[nodiscard]] Status SliceView(int32_t start, int32_t length,
                                 std::unique_ptr<ReadOnlyBatch>* out) const override {
    int32_t new_offset = offset_ + start;
    if (new_offset % 8 != 0) {
      return Status::Invalid(
          "Slicing a batch/array requires slicing 8 values at a time.  The offset must "
          "be divisible by 8 but was ",
          new_offset);
    }
    *out = std::make_unique<ViewBatch>(target_, column_indices_, view_schema_,
                                       offset_ + start, length);
    return Status::OK();
  }
 ...
 private:
  ...
  int32_t offset_ = 0;
  int32_t length_ = -1;
  const SimpleSchema* view_schema_;
};
...
[[nodiscard]] std::unique_ptr<ReadOnlyBatch> ReadOnlyBatch::SelectView(
  ...
  return std::make_unique<ViewBatch>(this, std::move(indices), new_schema, /*start=*/0,
                                     /*length=*/-1);
}

[[nodiscard]] Status ReadOnlyBatch::SliceView(int32_t start, int32_t length,
                                              std::unique_ptr<ReadOnlyBatch>* out) const {
  if (start % 8 != 0) {
    return Status::Invalid(
        "Batches/arrays must be sliced in groups of 8.  Offset must be divisible by 8 "
        "but was ",
        start);
  }
  *out = std::make_unique<ViewBatch>(this, /*column_indices=*/std::vector<int32_t>{},
                                     schema(), start, length);
  return Status::OK();
}
